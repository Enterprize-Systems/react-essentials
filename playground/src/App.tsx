import {classList, ForOf, If, Switch, SwitchCase, SwitchCaseDefault} from '@enterprize/react-essentials';
...
return (
  <div>
    ...
    <Switch expression={switchExampleValue}>
      ...
      <SwitchCase when={'value_3'}>
      	{() => <>Case: Value 3 selected</>}
			</SwitchCase>
...
return <ForOf
	...
	{/*Use object destructuring to read only what you need.*/}
	{({ item, index, length, isOdd, isEven, isFirst, isLast }) => <div>
			...
			<div className={classes.content}>
				...
				<article>
					...
					<div>
						<div className={classes.description}>
							<h3>Description</h3>
							<p>Use <code>Switch</code> to dynamically show or hide pieces of JSX
								using a <code>switch..case..default</code> approach.
								Use <code>SwitchCase</code> as <code>case</code> statement and
								optionally <code>SwitchCaseDefault</code> for <code>default</code> statement.
								The children
								of <code>SwitchCase</code> and <code>SwitchCaseDefault</code> can be
								a factory, similar to If.
							</p>
						</div>
						<div className={classes.props}>
							<h3>Props</h3>
						...
						<div style={{ display: 'flex', gap: '1rem' }}>
							<div className={classes.example} style={{ flexGrow: 1 }}>
								<h3>Example</h3>
								<div>
									<select
										onChange={event => setSwitchExampleValue(event.target.value)}>
					...
					<div>
							...
							<table>
								...
								<tbody>
									...
									<tr>
										...
										<td></td>
										<td>
											Factory for JSX.Element to be rendered. This function
											will be called with the iteration object with
											the <code>ForOfIteration&lt;T&gt;</code> shape.
										</td>
									</tr>
									<tr>
										...
										<td></td>
										<td>
											The attribute to be used as key or a factory (invoked
											with <code>ForOfIteration&lt;T&gt;</code>) to generate a
											value. If not set, will cause the React warning about
											keys. You can return the index provided on the iteration
											object as a fallback in case no other value is
											acceptable.
										</td>
									</tr>
									<tr>
									...
									<tr>
										...
										<td><code>false</code></td>
										<td>
											Flag that indicates to use the iteration "item" as key.
											Only use this if the item is a primitive.
										</td>
									</tr>
								</tbody>
							</table>
						...
						<div className={classes.props}>
							...
							<ul>
								<li>Type params:
									<ul>
										<li><code>T</code> - The type of each <code>item</code> in
											the iteration
										</li>
									</ul>
								</li>
								<li>Properties:
									<ul>
										<li><code>item: T</code> - The item in the current loop
											iteration.
										</li>
										<li><code>index: number</code> - The index position of the
											item.
										</li>
										<li><code>length: number</code> - The length of the array
											being iterated.
										</li>
										<li><code>isOdd: number</code> - If the index is odd.</li>
										<li><code>isEven: number</code> - If the index is even.</li>
										<li><code>isFirst: boolean</code> - If the index is the last
											position of the array.
										</li>
										<li><code>isLast: boolean</code> - If the index is the first
											position of the array.
										</li>
									</ul>
								</li>
						...
						<div className={classes.example}>
							...
							<div>
								...
								<ForOf
									...
									{/*The child of a ForOf must be a factory. It is invoked with the iteration information.*/}
									{/*Use object destructuring to read only what you need.*/}
									{({ item, index, length, isOdd, isEven, isFirst, isLast }) => (
										<div>
											{item} | Index {index} |
											Length {length} | Is
											Odd? {String(isOdd)} | Is
											Even {String(isEven)} | Is
											First? {String(isFirst)} | Is
											Last {String(isLast)}
										</div>
									)}
								</ForOf>
							...
							<div>
								<p>
									Object attribute as key. Array: {JSON.stringify(
										[
											{ id: 1, value: 'One' },
											{ id: 2, value: 'Two' },
											{ id: 3, value: 'Tree' }
							...
							<div>
								<p>
									Factory as key. Array: {JSON.stringify(
										[
											{ id: 1, value: 'One' },
											{ id: 2, value: 'Two' },
											{ id: 3, value: null }
				...
				<article>
					...
					<div>
						<div className={classes.description}>
							<h3>Description</h3>
							<p>Helper function to compile list of classes, give it be a string or a
								map to apply classes
								conditionally.
							</p>
						</div>
						<div className={classes.props}>
							<h3>Signature</h3>
							<code>function classList(...classes: Array&lt;string | ClassMap&gt;):
								string;</code>
						</div>
						<div className={classes.props}>
							<h3>Classes and Interfaces</h3>
							...
							<p>
								A hash where each key is the name of the class and the value is a
								boolean expression that when
								<code>true</code> indicates to apply the class
								and <code>false</code> to remove.
							</p>
						</div>
						<div className={classes.example}>
							<h3>Example</h3>
